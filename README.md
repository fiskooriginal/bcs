...
- **Управление скриптами**: Редактирование и синхронизация Python-скриптов из директории backend
- **Планирование по Cron**: Настройка расписания через cron-выражения с человекочитаемым описанием
- **Активация и деактивация**: Включение и отключение расписания для каждого скрипта
- **Ручной запуск**: Запуск скрипта в любой момент кнопкой «Run Now» и остановка выполняющегося процесса
- **Логи в реальном времени**: Потоковая передача stdout/stderr через WebSocket во время выполнения
- **История выполнений**: История всех запусков с бесконечной прокруткой (20 последних + подгрузка по скроллу)
- **Live View**: Открытие отдельного окна для просмотра логов выполняющегося скрипта в реальном времени
- **Monaco Editor**: Просмотр кода с подсветкой синтаксиса Python и копированием кода
- **REST API**: Backend на FastAPI с пагинацией, эндпоинтом подсчёта выполнений и полной документацией
...
- `POST /api/scripts/sync` — Синхронизировать скрипты из директории backend/scripts
- `GET /api/scripts/{id}` — Получить скрипт
- `PUT /api/scripts/{id}` — Обновить метаданные скрипта (cron_expression)
...
- `GET /api/scripts/{id}/executions` — История выполнений (поддержка `limit`, `offset`)
- `GET /api/scripts/{id}/executions/count` — Общее количество выполнений скрипта
